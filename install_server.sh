...
function set_metrics_refresher() {
  local -r REFRESH_SCRIPT="${STATE_DIR}/refresh_metrics.sh"
  # Путь к папке для удаления
  local -r TARGET_DIR="${STATE_DIR}/prometheus/data"
...
CURRENT_TIME=\$(TZ="Europe/Moscow" date +%s)
# Время 6:00 по Москве сегодняшнего дня в формате timestamp
TODAY_SIX_AM=\$(TZ="Europe/Moscow" date -d "6:00" +%s)
...
# Обновляем файл отметки времени
echo \$CURRENT_TIME > "${LAST_RUN_FILE}"
EOF
